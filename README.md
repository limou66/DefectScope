...
面向机器学习 / 视觉算法岗位的工程与实验作品。**它不是大模型训练项目，也不宣称提出了新的 SOTA 算法。** 核心检测、校准、评测和界面代码在本项目内独立实现，没有复制某个异常检测仓库。

![合成电路板示例](docs/assets/synthetic-gallery.png)

图中为本项目生成的合成数据。真实 MVTec 图片保留在本机 data 目录，不随代码发布。
...
- 项目不包含真实产线部署、大模型微调或线上业务收益，简历不应添加这些经历。
